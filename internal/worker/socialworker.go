...
func NewSocialWorker(ch *amqp.Channel, repo *repository.SocialRepository, queue string) *SocialWorker {
...
func (w *SocialWorker) process(ctx context.Context, body []byte) error {
	...
	switch evt.Action {
	case "follow":
		err := w.repo.Follow(ctx, &model.Social{
		...
		return err
	case "unfollow":
		return w.repo.Unfollow(ctx, &model.Social{
			FollowerID: evt.FollowerID,
			VloggerID:  evt.VloggerID,
		})
	default:
